...
<div class="w-full min-h-screen flex justify-center items-center bg-gradient-to-r from-red-700 to-white">
    <div class="max-w-[1300px] lg:w-10/12 w-full min-[1025px]:min-h-screen lg:m-5 flex overflow-hidden lg:bg-[#fff] lg:shadow-xl">
        <form method="POST" action="/register" class="lg:w-1/2 flex justify-center items-center w-full lg:p-3">
            <div class="sm:w-[450px] w-[90%] bg-[#fff] flex flex-col justify-center rounded-md p-6 shadow-xl gap-y-3">
                <div class="mb-2">
                    <h1 class="font-bold text-[27px]">JeiKei Register</h1>
                ...
                <input placeholder="••••••••" id="password-confirm" type="password" name="password_confirmation" class="<?= isset($errors['password']) ? 'border-red-500' : '' ?> block text-[#333f48] border border-[#333f48] p-[6px] focus:outline-none focus:border-sky-700 focus:ring-sky-700 rounded-md">
                <?php if (isset($errors['password'])) : ?>
                    <span class="text-red-500">
                        <i class="fa-solid fa-triangle-exclamation"></i>
                        <strong><?= $this->e($errors['password']) ?></strong>
                ...
                <input name="phone" value="<?= isset($old['phone']) ? $this->e($old['name']) : '' ?>" autofocus type="text" placeholder="0123456789" class="<?= isset($errors['phone']) ? 'border-red-500' : '' ?> block text-[#333f48] border border-[#333f48] p-[6px] focus:outline-none focus:border-sky-700 focus:ring-sky-700 rounded-md">
                ...
                <textarea name="address" maxlength="100" class="<?= isset($errors['address']) ? 'border-red-500' : '' ?> w-full block text-[#333f48] border border-[#333f48] p-[6px] focus:outline-none focus:border-sky-700 focus:ring-sky-700 rounded-md" placeholder="Địa chỉ ..."></textarea>
                ...
                <div class="flex items-center gap-1 justify-center">
                    <p class="text-[14px] text-[#333] font-semibold">Nếu đã có tài khoản?</p>
                    <a href="/login" class="text-[15px] text-[#5429FF] font-semibold cursor-pointer hover:underline">Đăng nhập</a>
                </div>
            </div>
        </form>
